from numpy import array, unravel_index, nditer, linalg, random, subtract, power
from numpy import exp, pi, zeros, arange, outer, meshgrid
...
class MiniSom:
    ...
    def _activate(self, x):
        """ Updates matrix activation_map, in this matrix the element i,j is
        the response of the neuron i,j to x """
        s = subtract(x, self.weights)  # x - w
        it = nditer(self.activation_map, flags=['multi_index'])
        while not it.finished:
            # || x - w ||
            self.activation_map[it.multi_index] = linalg.norm(s[it.multi_index]
                                                              )
            it.iternext()
    ...
    def update(self, x, win, t):
        ...
        while not it.finished:
            ...
            self.weights[it.multi_index] += g[it.multi_index] * (x -
                                                self.weights[it.multi_index])
            # normalization
            self.weights[it.multi_index] = (self.weights[it.multi_index] /
            linalg.norm(self.weights[it.multi_index]))
            it.iternext()
    ...
    def random_weights_init(self, data):
        """ Initializes the weights of the SOM picking random
        samples from data """
        it = nditer(self.activation_map, flags=['multi_index'])
        while not it.finished:
            self.weights[it.multi_index] = data[
                        int(self.random_generator.rand() * len(data) - 1)]
            self.weights[it.multi_index] = (self.weights[it.multi_index] /
            linalg.norm(self.weights[it.multi_index]))
            it.iternext()
        ...
        while not it.finished:
            for ii in range(it.multi_index[0] - 1, it.multi_index[0] + 2):
                for jj in range(it.multi_index[1] - 1, it.multi_index[1] + 2):
                    if (ii >= 0 and ii < self.weights.shape[0] and jj >= 0 and
                    jj < self.weights.shape[1]):
                        um[it.multi_index] += (linalg.norm(
    ...
    def win_map(self, data):
        """
            Returns a dictionary wm where wm[(i,j)] is a list with all
            the patterns
