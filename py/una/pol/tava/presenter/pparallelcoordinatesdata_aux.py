...
class WorkingPageDataPresenter:
    ...
    def fileExists(self, ite):
        return inm().fileExists(ite, self.iview.mode)

    def createFiles(self, ite):
        return inm().createFiles(ite, self.iview.mode)

    def createFilesWithFilter(self, ite, filters):
        return inm().createFilesWithFilter(ite, self.iview.mode, filters)

    def deleteFile(self, ite):
        return inm().deleteFile(ite, self.iview.mode)
    #-------------------------------------------------------------------------
